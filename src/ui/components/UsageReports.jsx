...
function TopApps({ sessions, colors, spacing }) {
  ...
  return (
    <div style={{ marginTop: `${spacing.base}px` }}>
      <div style={{ fontSize: '13px', fontWeight: '600', color: colors.text.secondary, marginBottom: `${spacing.sm}px` }}>{t('Top Apps')}</div>
      {sorted.map((app, i) => (
...
function DailySummary({ childPublicKey, colors, spacing, radius }) {
  ...
  return (
    <div style={{
      ...
      {loading ? (
        <div style={{ textAlign: 'center', color: colors.text.muted, fontSize: '14px', padding: `${spacing.lg}px 0` }}>{t('Loading...')}</div>
      ) : sessions.length === 0 ? (
        <div style={{ textAlign: 'center', color: colors.text.muted, fontSize: '14px', padding: `${spacing.lg}px 0` }}>{t('No usage data for this day.')}</div>
      ) : (
        <>
          {/* Summary stats */}
          <div style={{ display: 'flex', justifyContent: 'center', gap: `${spacing.xl}px`, marginBottom: `${spacing.base}px` }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '12px', color: colors.text.muted }}>{t('Screen Time')}</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '24px', fontWeight: '700', color: colors.text.primary }}>{sessionCount}</div>
              <div style={{ fontSize: '12px', color: colors.text.muted }}>{t('Sessions')}</div>
            </div>
          </div>
...
function WeeklyTrends({ childPublicKey, colors, spacing, radius }) {
  ...
  if (loading) return <div style={{ textAlign: 'center', color: colors.text.muted, fontSize: '14px', padding: `${spacing.lg}px 0` }}>{t('Loading...')}</div>;
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: `${spacing.xl}px`, marginBottom: `${spacing.base}px` }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '12px', color: colors.text.muted }}>{t('Daily Average')}</div>
        </div>
        {changePct !== null && (
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12px', color: colors.text.muted }}>{t('vs Previous')}</div>
          </div>
        )}
      </div>
      ...
      {summaries.length === 0 && (
        <div style={{ textAlign: 'center', color: colors.text.muted, fontSize: '14px', padding: `${spacing.lg}px 0` }}>{t('No trend data available yet.')}</div>
      )}
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', color: colors.text.muted, fontSize: '14px', padding: `${spacing.lg}px 0` }}>{t('No app usage for this day.')}</div>
      ) : (
        apps.map((app) => {
              ...
              {isExpanded && (
                <div style={{
                  ...
                  <div style={{ display: 'flex', gap: `${spacing.md}px`, padding: `${spacing.sm}px 0`, flexWrap: 'wrap' }}>
                    <div>
                      <div style={{ fontSize: '11px', color: colors.text.muted }}>{t('Sessions')}</div>
                      <div style={{ fontSize: '14px', fontWeight: '600', color: colors.text.primary }}>{appSessions.length}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: '11px', color: colors.text.muted }}>{t('Longest')}</div>
                      <div style={{ fontSize: '14px', fontWeight: '600', color: colors.text.primary }}>{formatSeconds(longestSession)}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: '11px', color: colors.text.muted }}>{t('Average')}</div>
                  ...
                  {sparkData[app.packageName] && (
                    <div style={{ padding: `${spacing.xs}px 0` }}>
                      <div style={{ fontSize: '11px', color: colors.text.muted, marginBottom: '4px' }}>{t('Last 7 days')}</div>
                  ...
                  <div style={{ marginTop: `${spacing.sm}px` }}>
                    <div style={{ fontSize: '11px', color: colors.text.muted, marginBottom: '4px' }}>{t('Sessions')}</div>
                    {appSessions.map((s, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: '13px' }}>
                          <span style={{ color: colors.text.secondary }}>{fmt(start)} - {end ? fmt(end) : t('now')}</span>
...
function CategoryBreakdown({ childPublicKey, colors, spacing, radius }) {
  ...
  return (
    <div>
      {/* Period toggle */}
      <div style={{ display: 'flex', justifyContent: 'center', gap: `${spacing.xs}px`, marginBottom: `${spacing.base}px` }}>
        {[{ value: 1, label: t('Today') }, { value: 7, label: t('7 days') }, { value: 30, label: t('30 days') }].map((p) => (
          <button
            key={p.value}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', color: colors.text.muted, fontSize: '14px', padding: `${spacing.lg}px 0` }}>{t('No category data for this day.')}</div>
      ) : (
        <>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: colors.surface.base }}>
      ...
      <div style={{
        ...
        <span style={{ ...typography.subheading, color: colors.text.primary, fontWeight: '600' }}>{t('Usage Reports')}</span>
      </div>
