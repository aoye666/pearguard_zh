    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: `${spacing.sm}px` }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', flex: 1 }}>
          ...
          {addedDate && <span style={{ fontSize: '11px', color: colors.text.muted, marginTop: '1px' }}>{t('apps.added')} {addedDate}</span>}
        </div>
        {override && (
          <span style={{
            fontSize: '11px',
            ...
            {timeRemaining(override.expiresAt)} {t('apps.timeLeft')}
          </span>
        )}
      </div>
      {isPending ? (
        <div style={{ display: 'flex', gap: `${spacing.sm}px` }}>
          <button
            ...
            aria-label={`${t('apps.approve')} ${appData.appName || packageName}`}
          >
            {t('apps.approve')}
          </button>
          <button
            ...
            onClick={handleDeny}
            aria-label={`${t('apps.deny')} ${appData.appName || packageName}`}
          >
            {t('apps.deny')}
          </button>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: `${spacing.base}px`, flexWrap: 'wrap' }}>
          <label style={{ display: 'flex', alignItems: 'center', fontSize: '13px', cursor: 'pointer', color: colors.text.primary }}>
            ...
            />
            <span style={{ marginLeft: '4px' }}>{appData.status === 'allowed' ? t('apps.allowed') : t('apps.blocked')}</span>
          </label>
          {inherits && !editingLimit ? (
            <span style={{ fontSize: '13px', color: colors.text.secondary, display: 'flex', alignItems: 'center', gap: '6px' }}>
              {t('apps.inherits')} {Math.round(categoryLimitSeconds / 60)} min/day
              <button
            ...
            <label style={{ fontSize: '13px', color: colors.text.secondary, display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              {hasOwnLimit && typeof categoryLimitSeconds === 'number' && !limitDirty && (
                <button
                  ...
                  title={t('apps.revertToCategory', { mins: Math.round(categoryLimitSeconds / 60) })}
                >
                  {t('apps.revert')}
                </button>
              )}
          ...
          <button
            ...
            aria-label={t('apps.timeWindowAria', { name: appData.appName || packageName })}
          >
            {windowSummary || t('apps.timeWindow')}
          </button>
        </div>
      )}
      <AppWindowModal
...
const STATUS_CATEGORIES = [
  { key: 'pending', label: t('apps.pendingApproval'), color: '#f59e0b' },
  { key: 'allowed', label: t('apps.allowed'),       color: '#34a853' },
  { key: 'blocked', label: t('apps.blocked'),        color: '#ea4335' },
];
        ...
        <div style={batchAnimationStyle}>
          {batchAction && (
            <div style={{ display: 'flex', gap: `${spacing.sm}px`, padding: `${spacing.sm}px 0`, borderBottom: `1px solid ${colors.divider}` }}>
              ...
              >
                {batchAction === 'approve' ? t('apps.approveAll') : t('apps.denyAll')}
              </button>
            </div>
          )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: `${spacing.sm}px` }}>
        <label style={{ fontSize: '13px', color: colors.text.secondary, display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <input
            ...
            style={{ width: '60px', padding: '4px', border: `1px solid ${colors.border}`, borderRadius: `${radius.sm}px`, fontSize: '13px', backgroundColor: colors.surface.input, color: colors.text.primary }}
            placeholder="&#8734;"
            aria-label={t('apps.dailyLimitCategory')}
          />
          {t('apps.minPerDay')}
        </label>
        {dirty && (
        ...
        <div style={batchAnimationStyle}>
          {pendingCount > 0 && (
            <div style={{ display: 'flex', gap: `${spacing.sm}px`, padding: `${spacing.sm}px 0`, borderBottom: `1px solid ${colors.divider}` }}>
              ...
              >
                {t('apps.approveAllPending', { count: pendingCount })}
              </button>
              <button
                style={{ flex: 1, padding: `${spacing.sm}px ${spacing.md}px`, border: 'none', borderRadius: `${radius.md}px`, backgroundColor: colors.error, color: '#FFFFFF', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}
                onClick={() => { window.callBare('haptic:tap'); handleDenyPending(); }}
              >
                {t('apps.denyAllPending', { count: pendingCount })}
              </button>
            </div>
          )}
...
export default function AppsTab({ childPublicKey }) {
  ...
  function handleCategoryApply(categoryName) {
    ...
    setConfirmDialog({
      kind: 'apply',
      categoryName,
      title: t('apps.applyToCategoryTitle', { mins: catLimit.dailyLimitSeconds / 60, category: categoryName }),
      message: t('apps.applyToCategoryMsg', { count: affected, category: categoryName }),
      confirmLabel: t('apps.apply'),
      action: () => {
        persistPolicy(prev => {
  ...
  function handleCategoryClear(categoryName) {
    ...
    setConfirmDialog({
      kind: 'clear',
      categoryName,
      title: t('apps.clearCategoryTitle', { category: categoryName }),
      message: affected > 0
        ? t('apps.clearCategoryMsgWithApps', { count: affected, category: categoryName })
        : t('apps.clearCategoryMsgNoApps', { category: categoryName }),
      confirmLabel: t('apps.clear'),
      action: () => {
        persistPolicy(prev => {
          const categories = { ...(prev.categories || {}) };
          delete categories[categoryName];
  ...
  if (loading) return <div style={{ padding: `${spacing.base}px`, color: colors.text.muted, fontSize: '14px' }}>{t('apps.loading')}</div>;
  if (!policy || !policy.apps || Object.keys(policy.apps).length === 0) {
    return <div style={{ padding: `${spacing.base}px`, color: colors.text.muted, fontSize: '14px' }}>{t('apps.noApps')}</div>;
  }
    ...
    <div style={{ padding: `0 ${spacing.base}px ${spacing.base}px` }}>
        ...
        <div style={{ display: 'flex', gap: `${spacing.md}px`, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: colors.text.muted }}>
            ...
            <button
              ...
              aria-label={t('apps.toggleSort')}
            >
              {sortOrder === 'alpha' ? t('apps.sortByName') : t('apps.sortByDate')}
            </button>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: colors.text.muted }}>
            {t('apps.filterBy')}
            <button
              ...
              aria-label={t('apps.toggleViewMode')}
            >
              {viewMode === 'status' ? t('apps.status') : t('apps.category')}
            </button>
          </label>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: `${spacing.sm}px`, marginBottom: `${spacing.md}px` }}>
        <span style={{ fontSize: '13px', color: colors.text.muted }}>
          {q ? `${visibleCount} ${t('apps.of')} ${totalCount}` : totalCount} {t('apps.app', { count: totalCount })}
        </span>
        {q && (
          <button
      ...
      <Modal
        ...
        footer={<>
          <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); setConfirmDialog(null); }} style={{ flex: 1 }}>{t('common.cancel')}</Button>
          <Button variant="danger" onClick={() => { window.callBare('haptic:tap'); runConfirm(); }} style={{ flex: 1 }}>{confirmDialog?.confirmLabel || t('common.confirm')}</Button>
        </>}
      >
