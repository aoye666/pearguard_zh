...
export default forwardRef(function Dashboard(_props, ref) {
  ...
  return (
    <div style={{ padding: `${spacing.base}px` }}>
      ...
      {loading && <p style={{ ...typography.body, color: colors.text.secondary }}>{t('Loading...')}</p>}
      ...
      <Modal
        ...
        title={t(`Lock ${lockTarget?.displayName}'s device?`)}
        footer={<>
          <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); setLockTarget(null); setLockMessage(''); }} style={{ flex: 1 }}>{t('Cancel')}</Button>
          <Button variant="danger" icon="LockSimple" onClick={() => { window.callBare('haptic:tap'); confirmLock(); }} style={{ flex: 1 }}>{t('Lock')}</Button>
        </>}
      >
        <div style={{ textAlign: 'center', marginBottom: `${spacing.md}px` }}>
          {t(`This will immediately block all apps on ${lockTarget?.displayName}'s device until you unlock it.`)}
        </div>
        <Input
