  ...
  return (
    <div>
      ...
      >
        ...
        >
          {copied ? <><Icon name="CheckCircle" size={14} weight="fill" color={colors.success} /> {t('Copied')}</> : t('Copy')}
        </button>
      </div>
      {hint && (
...
export default function AboutTab() {
  ...
  return (
    <div style={{ padding: `${spacing.base}px`, overflowY: 'auto', flex: 1 }}>
      ...
      <Collapsible title={t('Tutorial')} icon="BookOpen" open={tutorialOpen} onToggle={() => setTutorialOpen(v => !v)} maxHeight="200px" {...collapsibleProps}>
        <p style={bodyStyle}>
          {t('Replay the in-app walkthrough that introduces the Dashboard, Apps, Rules, and Activity tabs.')}
        </p>
        <Button variant="accent" onClick={() => { window.callBare('haptic:tap'); window.__pearReplayTour?.(); }} style={fullWidth}>
          <Icon name="BookOpen" size={16} color="#000000" /> {t('Replay Tutorial')}
        </Button>
      </Collapsible>
      ...
      <Collapsible title={t('Support development')} icon="Lightning" open={valueOpen} onToggle={() => setValueOpen(v => !v)} maxHeight="200px" {...collapsibleProps}>
        ...
        <div style={{ display: 'flex', gap: `${spacing.sm}px` }}>
          <Button variant="accent" onClick={handleDonateBTC} style={flexOne}>
            <Icon name="Lightning" size={14} color="#000000" /> {t('BTC')} <Icon name="Lightning" size={14} color="#000000" />
          </Button>
          <Button variant="accent" onClick={() => openURL('https://buymeacoffee.com/peerloomllc')} style={flexOne}>
            <Icon name="CurrencyDollar" size={14} color="#000000" /> {t('USD')} <Icon name="CurrencyDollar" size={14} color="#000000" />
          </Button>
        </div>
      </Collapsible>

      <Collapsible title={t('Learn about Bitcoin')} icon="BookOpen" open={bitcoinOpen} onToggle={() => setBitcoinOpen(v => !v)} maxHeight="220px" {...collapsibleProps}>
        <p style={bodyStyle}>
          {t('New to Bitcoin? The Satoshi Nakamoto Institute has a free, concise crash course explaining how Bitcoin works and why it matters.')}
        </p>
        <Button variant="accent" onClick={() => openURL('https://nakamotoinstitute.org/crash-course/')} style={fullWidth}>
          <Icon name="BookOpen" size={16} color="#000000" /> {t('Bitcoin Crash Course')} <Icon name="ArrowSquareOut" size={14} color="#000000" />
        </Button>
      </Collapsible>
      ...
      <Collapsible title={t('Contact')} icon="EnvelopeSimple" open={contactOpen} onToggle={() => setContactOpen(v => !v)} maxHeight="120px" {...collapsibleProps}>
        <div style={{ display: 'flex', gap: `${spacing.sm}px`, justifyContent: 'center' }}>
          ...
          >
            <Icon name="Bug" size={14} color="#000000" /> {t('Report Issue')} <Icon name="ArrowSquareOut" size={13} color="#000000" />
          </Button>
        </div>
      </Collapsible>
      ...
      {walletModal && (
        <BottomSheet
          ...
          title={<><Icon name="Lightning" size={18} color={colors.primary} /> {t('Bitcoin Lightning')} <Icon name="Lightning" size={18} color={colors.primary} /></>}
          footer={(close) => (
            <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); close(); }} style={fullWidth}>{t('Close')}</Button>
          )}
        >
          {(close) => (
            <>
              ...
              {lnDetected && (
                <>
                  <Button variant="accent" onClick={() => { openURL('lightning:' + LIGHTNING_ADDRESS); close(); }} style={optionBox}>
                    <Icon name="Lightning" size={16} color="#000000" /> {t('Open in your Lightning wallet')} <Icon name="Lightning" size={16} color="#000000" />
                  </Button>
                  <p style={{ lineHeight: '1.7', textAlign: 'center', margin: `${spacing.base}px 0 0` }}>{t('or use another method:')}</p>
                </>
              )}

              <p style={{ ...secLabel, marginTop: `${lnDetected ? spacing.base : spacing.md}px` }}>{t('Lightning address')}</p>
              <CopyField value={LIGHTNING_ADDRESS} hint={t('Paste into any Lightning, ecash or web wallet.')} />

              <div style={{ marginTop: `${spacing.base}px` }}>
                <Button variant="accent" onClick={() => { openURL(STRIKE_TIP_URL); close(); }} style={optionBox}>
                  <Icon name="Lightning" size={16} color="#000000" /> {t('Show a QR / pay in a browser')} <Icon name="Lightning" size={16} color="#000000" />
                </Button>
                <p style={{ ...typography.caption, color: colors.text.muted, margin: `${spacing.xs}px 0 0`, textAlign: 'center', lineHeight: '1.5' }}>
                  {t('Scan from another device or on desktop.')}
                </p>
              </div>

              {BTC_ONCHAIN_ADDRESS && (
                <>
                  <p style={secLabel}>{t('On-chain Bitcoin')}</p>
                  <CopyField value={BTC_ONCHAIN_ADDRESS} hint={t('On-chain BTC. Higher fees, so Lightning is cheaper for small tips.')} />
                </>
              )}
              ...
              {!lnDetected && (
                <>
                  <p style={{ lineHeight: '1.7', textAlign: 'center', margin: `${spacing.lg}px 0 ${spacing.sm}px` }}>
                    {t('Don\'t have a Lightning wallet?')}
                  </p>
                  ...
                  <p style={{ lineHeight: '1.7', textAlign: 'center', marginTop: `${spacing.base}px`, marginBottom: 0 }}>
                    {t('After installing, return here and tap BTC again.')}
                  </p>
                </>
              )}
            </>
