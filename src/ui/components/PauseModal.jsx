...
export default function PauseModal({ child, pauseUntil, visible, onClose, onChanged }) {
  ...
  return (
    <Modal
      visible={visible}
      onClose={onClose}
      title={isPaused ? t('{name} is on free time', { name: child?.displayName }) : t('Pause protection for {name}?', { name: child?.displayName })}
      footer={<Button variant="secondary" disabled={busy} onClick={() => { window.callBare('haptic:tap'); onClose(); }} style={{ flex: 1 }}>{t('Close')}</Button>}
    >
      {isPaused ? (
        <>
          <div style={{ textAlign: 'center', marginBottom: `${spacing.md}px`, ...typography.body, color: colors.text.primary }}>
            {t('All limits, schedules and blocks are suspended until {time}.', { time: formatUntil(pauseUntil) })}
          </div>
          <Button variant="danger" icon="LockSimple" disabled={busy} onClick={() => setPause(0)} style={{ width: '100%' }}>
            {t('Resume protection now')}
          </Button>
        </>
      ) : (
        <>
          <div style={{ textAlign: 'center', marginBottom: `${spacing.md}px`, ...typography.caption, color: colors.text.secondary }}>
            {t('Temporarily allow every app - no limits, schedules or blocks - for a sick day, trip or reward. Protection resumes automatically.')}
          </div>
