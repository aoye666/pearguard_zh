    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, pointerEvents: 'none' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: `${spacing.xs}px` }}>
          <span style={{ ...typography.caption, color: colors.text.muted }}>{index + 1} {t('of')} {total}</span>
          <button
            onClick={() => { window.callBare('haptic:tap'); skip(); }}
            aria-label={t('Skip tutorial')}
            style={{
        ...
        <div style={{ display: 'flex', gap: `${spacing.sm}px` }}>
          ...
          <button
            ...
            aria-label={isLast ? t('Finish tutorial') : t('Next')}
            style={{
              flex: 1,
              padding: `${spacing.sm}px`,
              border: 'none',
          ...
          >
            {isLast ? (currentSlide.cta || t('Get Started')) : t('Next')}
          </button>
        </div>
      </div>
    </div>
