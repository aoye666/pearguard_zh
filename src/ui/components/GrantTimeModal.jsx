...
export default function GrantTimeModal({ child, visible, onClose }) {
  ...
  return (
    <Modal
      visible={visible}
      onClose={close}
      title={grantedSeconds ? t('Bonus time granted') : t('Grant bonus time to {name}?', { name: child?.displayName })}
    ...
    >
      {grantedSeconds ? (
        <div style={{ textAlign: 'center', ...typography.body, color: colors.text.primary }}>
          {t('Added {time} to {name}\'s screen time for today.', { time: grantLabel(grantedSeconds), name: child?.displayName })}
        </div>
      ) : (
        <>
          <div style={{ textAlign: 'center', marginBottom: `${spacing.md}px`, ...typography.caption, color: colors.text.secondary }}>
            {t('Tops up today\'s screen-time budget. Schedules, per-app limits and blocked apps still apply.')}
          </div>
