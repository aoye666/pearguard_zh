...
import { validatePin, MIN_PIN_LENGTH, MAX_PIN_LENGTH } from '../../pin-rules.js';
...
function StorageModal({ modal, onClose, breakdown, analyze, result, colors, typography, spacing, radius, formatBytes }) {
  ...
  const title = modal === 'breakdown' ? t('Storage Breakdown')
    : modal === 'analyze' ? t('Reclaimable Analysis')
    : t('Reclaim Complete');
        ...
        {modal === 'breakdown' && breakdown && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: `${spacing.md}px` }}>
              <span style={{ fontSize: 13, color: colors.text.secondary }}>{t('On disk')}</span>
        ...
        {modal === 'analyze' && analyze && (
          <div>
            <div style={{ padding: `${spacing.md}px`, backgroundColor: colors.surface.base, borderRadius: `${radius.md}px`, marginBottom: `${spacing.md}px`, textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Reclaimable')}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: `${spacing.sm}px`, marginBottom: `${spacing.md}px`, fontSize: 12, textAlign: 'center' }}>
              <div><div style={{ color: colors.success, fontSize: 18, fontWeight: 600 }}>{analyze.groups.keep || 0}</div><div style={{ color: colors.text.muted }}>{t('keep')}</div></div>
              <div><div style={{ color: colors.error, fontSize: 18, fontWeight: 600 }}>{analyze.groups.wipe || 0}</div><div style={{ color: colors.text.muted }}>{t('wipe')}</div></div>
              <div><div style={{ color: colors.text.primary, fontSize: 18, fontWeight: 600 }}>{analyze.groups.request || 0}</div><div style={{ color: colors.text.muted }}>{t('requests')}</div></div>
            </div>
            <div style={{ fontSize: 12, color: colors.text.muted, marginBottom: `${spacing.sm}px` }}>{t('By prefix (live data)')}</div>
        ...
        {modal === 'result' && result && (
          <div>
            <div style={{ padding: `${spacing.md}px`, backgroundColor: colors.surface.base, borderRadius: `${radius.md}px`, textAlign: 'center', marginBottom: `${spacing.md}px` }}>
              <div style={{ fontSize: 11, color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.5 }}>{t('Freed')}</div>
              <div style={{ fontSize: 28, fontWeight: 700, color: colors.success, marginTop: 4 }}>{formatBytes(result.freed)}</div>
              <div style={{ fontSize: 13, color: colors.text.muted, marginTop: 4 }}>{formatBytes(result.before)} {t('to')} {formatBytes(result.after)}</div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: `${spacing.sm}px`, fontSize: 13, color: colors.text.secondary, textAlign: 'center' }}>
              <div><div style={{ color: colors.success, fontSize: 18, fontWeight: 600 }}>{result.kept}</div><div>{t('keys kept')}</div></div>
              <div><div style={{ color: colors.error, fontSize: 18, fontWeight: 600 }}>{result.dropped}</div><div>{t('keys dropped')}</div></div>
            </div>
          </div>
        )}

        <div style={{ marginTop: `${spacing.lg}px`, display: 'flex', justifyContent: 'center' }}>
          <button onClick={onClose} style={{ padding: `${spacing.sm}px ${spacing.xl}px`, borderRadius: `${radius.md}px`, border: `1px solid ${colors.border}`, background: 'transparent', color: colors.text.primary, fontSize: 14, cursor: 'pointer', minWidth: 140 }}>{t('Close')}</button>
        </div>
      </div>
    </div>
  );
...
export default function Settings() {
  ...
  function handlePinSubmit(e) {
    ...
    if (newPin !== confirmPin) {
      setPinStatus(t('PINs do not match.'));
      return;
    }
    setPinStatus(null);
    window.callBare('pin:set', { pin: newPin })
      ...
      .catch((err) => {
        setPinStatus(err.message || t('Failed to set PIN. Please try again.'));
      });
  }

  async function handleSettingsSave() {
    if (timeRequestMinutes.length === 0 || warningMinutes.length === 0) {
      setSettingsStatus(t('Select at least one option for each setting.'));
      return;
    }
    ...
    try {
      ...
      setSettingsStatus(t('Failed to save settings.'));
    }
  }
  ...
  async function handleStorageBreakdown() {
    ...
    try {
      ...
      setStorageError(e.message || t('Failed to read storage'));
    }
  ...
  async function handleStorageAnalyze() {
    ...
    try {
      ...
      setStorageError(e.message || t('Failed to analyze storage'));
    }
    setStorageBusy(false);
  }
  ...
  async function handleStorageReclaim() {
    ...
    try {
      ...
      setStorageError(e.message || t('Reclaim failed'));
    }
    setStorageBusy(false);
  }
  ...
  return (
    <div style={{ padding: `${spacing.base}px`, overflowY: 'auto', flex: 1 }}>
      <h2 style={{ ...typography.heading, color: colors.text.primary, marginBottom: `${spacing.lg}px`, textAlign: 'center' }}>{t('Settings')}</h2>
      ...
      <section style={{ marginBottom: `${spacing.xxl}px` }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: `${spacing.lg}px` }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: `${spacing.sm}px` }}>
            ...
            >
              <Icon name="ImageSquare" size={14} color={colors.text.primary} /> {t('Photo')}
            </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.sm}px` }}>
          ...
          {savingName && <p style={{ color: colors.text.muted, fontSize: '13px', margin: 0 }}>{t('Saving...')}</p>}
          {!savingName && nameStatus === 'success' && <p style={{ color: colors.success, fontSize: '13px', margin: 0 }}>{t('Saved.')}</p>}
          {nameStatus === 'error' && <p style={{ color: colors.error, fontSize: '13px', margin: 0 }}>{t('Failed to save name.')}</p>}
        </div>
      </section>

      {/* Override PIN */}
      <div data-tour-id="settings-override-pin">
      <Collapsible title={t('Override PIN')} icon="LockSimple" open={pinOpen} onToggle={() => setPinOpen(o => !o)} maxHeight="350px" {...collapsibleProps}>
        <p style={{ fontSize: '12px', color: colors.text.muted, marginBottom: `${spacing.md}px`, marginTop: 0 }}>
          {t('Children enter this PIN on the block overlay to get temporary access. The PIN is hashed before leaving this device.')}
        </p>
        {currentPin && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: `${spacing.sm}px`, padding: `${spacing.sm}px ${spacing.md}px`, backgroundColor: colors.surface.base, borderRadius: `${radius.md}px`, marginBottom: `${spacing.md}px` }}>
            <span style={{ fontSize: '14px', color: colors.text.secondary }}>
              {t('Current PIN: ')}<span style={{ fontFamily: 'monospace', fontSize: '16px', color: colors.text.primary, letterSpacing: '2px' }}>{pinRevealed ? currentPin : '••••'}</span>
            </span>
            <button
              type="button"
              ...
              aria-label={pinRevealed ? t('Hide current PIN') : t('Show current PIN')}
            >
              <Icon name={pinRevealed ? 'EyeSlash' : 'Eye'} size={18} color={colors.text.muted} />
              {pinRevealed ? t('Hide') : t('Show')}
            </button>
          </div>
        )}
        <form onSubmit={handlePinSubmit} style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.md}px` }} aria-label={t('Change PIN form')}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.xs}px`, fontSize: '14px', color: colors.text.secondary }}>
            {t('New PIN (4 to {max} digits)', { max: MAX_PIN_LENGTH })}
            <input
              type="text"
              value={newPin}
              onChange={(e) => {
          ...
          {pinStatus === 'success' && (
            <p style={{ color: colors.success, fontSize: '13px', margin: 0 }} role="status">{t('PIN updated successfully.')}</p>
          )}
          <Button type="submit" style={{ alignSelf: 'center' }} aria-label={t('Save PIN')}>
            {t('Save PIN')}
          </Button>
        </form>
      </Collapsible>
      ...
      {/* Appearance */}
      {settingsLoaded && (
        <Collapsible title={t('Appearance')} icon="SunDim" open={appearanceOpen} onToggle={() => setAppearanceOpen(o => !o)} maxHeight="200px" {...collapsibleProps}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: `${spacing.sm}px` }}>
              ...
              <span style={{ fontSize: '14px', color: colors.text.primary }}>{currentTheme === 'dark' ? t('Dark mode') : t('Light mode')}</span>
            </div>
            <Toggle checked={currentTheme === 'dark'} onChange={(checked) => setTheme(checked ? 'dark' : 'light')} />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: `${spacing.sm}px` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: `${spacing.sm}px` }}>
              <Icon name="Translate" size={20} color={colors.text.primary} />
              <span style={{ fontSize: '14px', color: colors.text.primary }}>{t('Language')}</span>
            </div>
            <div style={{ display: 'flex', gap: `${spacing.xs}px` }}>
              <button
                onClick={() => { window.callBare('haptic:tap'); setLocale('zh-CN'); }}
                style={{
                  padding: `${spacing.xs}px ${spacing.sm}px`,
                  borderRadius: `${radius.sm}px`,
                  border: `1px solid ${getLocale() === 'zh-CN' ? colors.primary : colors.border}`,
                  background: getLocale() === 'zh-CN' ? colors.primary : 'transparent',
                  color: getLocale() === 'zh-CN' ? '#FFFFFF' : colors.text.primary,
                  fontSize: '13px', cursor: 'pointer', fontWeight: getLocale() === 'zh-CN' ? '600' : '400',
                }}
              >中文</button>
              <button
                onClick={() => { window.callBare('haptic:tap'); setLocale('en'); }}
                style={{
                  padding: `${spacing.xs}px ${spacing.sm}px`,
                  borderRadius: `${radius.sm}px`,
                  border: `1px solid ${getLocale() === 'en' ? colors.primary : colors.border}`,
                  background: getLocale() === 'en' ? colors.primary : 'transparent',
                  color: getLocale() === 'en' ? '#FFFFFF' : colors.text.primary,
                  fontSize: '13px', cursor: 'pointer', fontWeight: getLocale() === 'en' ? '600' : '400',
                }}
              >English</button>
            </div>
          </div>
      ...
      {settingsLoaded && (
        <Collapsible title={t('Time Request Options')} icon="Clock" open={timeOptsOpen} onToggle={() => setTimeOptsOpen(o => !o)} maxHeight="200px" {...collapsibleProps}>
          <p style={{ fontSize: '12px', color: colors.text.muted, marginBottom: `${spacing.md}px`, marginTop: 0 }}>
            {t('Choose which duration options the child sees when requesting more time from the block overlay.')}
          </p>
          <ChipSelect
      ...
      {/* Warning Thresholds */}
      {settingsLoaded && (
        <Collapsible title={t('Warning Notifications')} icon="Bell" open={warningOpen} onToggle={() => setWarningOpen(o => !o)} maxHeight="200px" {...collapsibleProps}>
          <p style={{ fontSize: '12px', color: colors.text.muted, marginBottom: `${spacing.md}px`, marginTop: 0 }}>
            {t('The child will be notified this many minutes before a schedule block starts or a daily time limit runs out.')}
          </p>
          <ChipSelect
      ...
      {/* Device Backup */}
      {settingsLoaded && (
        <Collapsible title={t('Device Backup')} icon="Export" open={backupOpen} onToggle={() => setBackupOpen(o => !o)} maxHeight="220px" {...collapsibleProps}>
          <div style={{ fontSize: '13px', color: colors.text.muted, marginBottom: `${spacing.sm}px` }}>
            {t('Save your full parent state (identity, children, policies) to migrate to a new device. To restore a backup, install the app on a fresh device and choose "Restore parent from backup" on the welcome screen.')}
          </div>
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <Button variant="secondary" icon="Export" onClick={() => { window.callBare('haptic:tap'); setBackupMode('export'); }}>{t('Export backup')}</Button>
          </div>
        </Collapsible>
      )}
      ...
      {/* Storage */}
      <Collapsible title={t('Storage')} icon="Trash" open={storageOpen} onToggle={() => setStorageOpen(o => !o)} maxHeight="400px" {...collapsibleProps}>
        <p style={{ fontSize: '12px', color: colors.text.muted, marginBottom: `${spacing.md}px`, marginTop: 0 }}>
          {t('The local database grows over time as telemetry (usage reports, alerts, sessions) accumulates. Check the current footprint and reclaim disk when it gets large.')}
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.sm}px`, alignItems: 'center' }}>
          <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); handleStorageBreakdown(); }} disabled={storageBusy} style={{ minWidth: '200px' }}>{t('Storage Breakdown')}</Button>
          <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); handleStorageAnalyze(); }} disabled={storageBusy} style={{ minWidth: '200px' }}>{t('Analyze Reclaimable')}</Button>
          <Button onClick={() => { window.callBare('haptic:tap'); setReclaimConfirm(true); }} disabled={storageBusy || !storageAnalyze} style={{ minWidth: '200px' }}>{t('Reclaim Storage')}</Button>
        </div>
        {storageBusy && <p style={{ fontSize: '13px', color: colors.text.muted, margin: `${spacing.md}px 0 0`, textAlign: 'center' }}>{t('Working...')}</p>}
      ...
      {reclaimConfirm && (
        <div onClick={() => setReclaimConfirm(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: `${spacing.lg}px` }}>
          <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: colors.surface.card, borderRadius: `${radius.lg}px`, padding: `${spacing.lg}px`, maxWidth: 420, width: '100%', border: `1px solid ${colors.border}` }}>
            <h3 style={{ ...typography.subheading, color: colors.text.primary, margin: 0, marginBottom: `${spacing.md}px`, textAlign: 'center' }}>{t('Reclaim Storage?')}</h3>
            <p style={{ fontSize: '13px', color: colors.text.secondary, margin: 0, marginBottom: `${spacing.lg}px`, lineHeight: 1.5, textAlign: 'center' }}>
              {t('This permanently deletes historical usage reports, alerts, override grants, bypass events and session logs. Identity, children, policies and pending requests are preserved.')}
            </p>
            <div style={{ display: 'flex', gap: `${spacing.sm}px`, justifyContent: 'center' }}>
              <Button variant="secondary" onClick={() => setReclaimConfirm(false)} style={{ flex: 1, maxWidth: 160 }}>{t('Cancel')}</Button>
              <Button onClick={() => { window.callBare('haptic:tap'); handleStorageReclaim(); }} style={{ flex: 1, maxWidth: 160 }}>{t('Reclaim now')}</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {settingsLoaded && (
        <div style={{ textAlign: 'center', marginTop: `${spacing.md}px`, marginBottom: `${spacing.xxl}px` }}>
          ...
          {settingsStatus === 'success' && (
            <p style={{ color: colors.success, fontSize: '13px', marginBottom: `${spacing.sm}px` }}>{t('Settings saved and synced to child.')}</p>
          )}
          <Button
