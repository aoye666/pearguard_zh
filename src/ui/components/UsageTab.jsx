    ...
    <div style={{ marginBottom: `${spacing.lg}px` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
        ...
        {onHide && (
          <button
            onClick={onHide}
            title={t("Hide from usage")}
            aria-label={t("Hide {{appName}} from usage", { appName })}
      ...
      <div>
        <div style={{ fontSize: '12px', color: colors.text.secondary, marginBottom: '3px', marginTop: '6px' }}>{t('Today:')} {formatSeconds(todaySeconds)}{dailyLimitSeconds ? `${t(' of ')}${formatSeconds(dailyLimitSeconds)}` : ''}</div>
        ...
        <div style={{ fontSize: '12px', color: colors.text.secondary, marginBottom: '3px', marginTop: '6px' }}>{t('Last 7 days:')} {formatSeconds(weekSeconds)}</div>
        <div style={{ height: '8px', backgroundColor: colors.surface.elevated, borderRadius: `${radius.sm}px`, overflow: 'hidden' }}>
          <div
            style={{
              height: '100%',
...
export default function UsageTab({ childPublicKey, onShowReports }) {
  ...
  if (loading) return <div style={{ padding: `${spacing.base}px`, color: colors.text.muted, fontSize: '14px' }}>{t('Loading usage data...')}</div>;
  ...
  return (
    <div style={{ padding: `${spacing.base}px` }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: `${spacing.xs}px`, marginBottom: `${spacing.base}px` }}>
        {onShowReports && (
          <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); onShowReports(); }} style={{ fontSize: '12px', padding: '4px 12px' }}>{t('See Reports')}</Button>
        )}
        {report && <p style={{ fontSize: '12px', color: colors.text.muted, margin: 0 }}>{t('Last synced:')} {timeAgo(report.lastSynced || report.timestamp)}</p>}
      </div>

      {!hasApps && (
        <div style={{ color: colors.text.muted, fontSize: '14px', textAlign: 'center', marginBottom: `${spacing.base}px` }}>
          t("No usage data yet. Data syncs every 15 minutes.")
        </div>
      )}
      ...
      {hidden.length > 0 && (
        <div style={{ marginTop: `${spacing.lg}px`, paddingTop: `${spacing.base}px`, borderTop: `1px solid ${colors.border}` }}>
          ...
          >
            ...
            {t('{{count}} hidden', { count: hidden.length })} {t(hidden.length === 1 ? 'app' : 'apps')}
          </button>
          {showHidden && (
