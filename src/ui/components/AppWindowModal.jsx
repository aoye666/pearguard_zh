...
export default function AppWindowModal({ appName, window: win, visible, onClose, onSave }) {
  ...
  function save() {
    if (!days.length) { setErr(t('Pick at least one day.')); return; }
    if (!start || !end) { setErr(t('Set a start and end time.')); return; }
    if (start === end) { setErr(t('Start and end can\'t be the same.')); return; }
  ...
  return (
    <Modal
      visible={visible}
      onClose={onClose}
      title={t('Time window - {appName}', { appName })}
      footer={<>
        {win && win.mode && <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); clear(); }} style={{ flex: 1 }}>{t('Remove')}</Button>}
        <Button onClick={() => { window.callBare('haptic:tap'); save(); }} style={{ flex: 1 }}>{t('Save')}</Button>
      </>}
    >
      <div style={{ display: 'flex', gap: `${spacing.sm}px`, marginBottom: `${spacing.md}px` }}>
        {modeBtn('block', t('Blocked during'))}
        {modeBtn('allow', t('Allowed only'))}
      </div>
      <div style={{ ...typography.caption, color: colors.text.secondary, marginBottom: `${spacing.md}px` }}>
        {mode === 'allow'
          ? t('{app} is blocked except during this window.', { app: appName })
          : t('{app} is blocked during this window.', { app: appName })}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: `${spacing.sm}px`, justifyContent: 'center' }}>
        <label style={{ ...typography.caption, color: colors.text.secondary }}>
          {t('From')} <input type="time" aria-label={t('Start time')} value={start} onChange={(e) => setStart(e.target.value)} style={inputStyle} />
        </label>
        <label style={{ ...typography.caption, color: colors.text.secondary }}>
          {t('to')} <input type="time" aria-label={t('End time')} value={end} onChange={(e) => setEnd(e.target.value)} style={inputStyle} />
        </label>
      </div>
