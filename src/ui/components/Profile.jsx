...
export default function Profile({ mode }) {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      window.onBareEvent('peer:paired', () => {
        ...
        refresh()
        setPairState('idle')
        setPairedBanner(t('Successfully paired with parent!'))
  ...
  function settleAfterAcceptInvite(result) {
    if (result && result.alreadyPaired) {
      setPairState('idle')
      setPairedBanner(t('Already paired with this parent.'))
      setTimeout(() => setPairedBanner((m) => (m === t('Already paired with this parent.') ? null : m)), 3000)
      return
    }
  ...
  const label = mode === 'parent' ? t('Parent Name') : t('Your Name')
  const placeholder = mode === 'parent' ? t('e.g. Mom') : t('e.g. Alex')
  ...
  return (
    <div style={{ padding: `${spacing.base}px` }}>
      <h2 style={{ ...typography.heading, color: colors.text.primary, marginBottom: `${spacing.xl}px`, textAlign: 'center' }}>{t('Profile')}</h2>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.sm}px` }}>
        ...
        {saving && <p style={{ color: colors.text.muted, fontSize: '13px', margin: 0 }}>{t('Saving...')}</p>}
        {!saving && status === 'success' && <p style={{ color: colors.success, fontSize: '13px', margin: 0 }}>{t('Saved.')}</p>}
        {status === 'error' && <p style={{ color: colors.error, fontSize: '13px', margin: 0 }}>{t('Failed to save name.')}</p>}
      </div>

      {mode === 'child' && (
        <div style={{ marginTop: `${spacing.xl}px` }}>
          <Collapsible
            title={t('Paired Parents')}
            icon="User"
            open={parentsOpen}
          ...
          >
              ...
              <div style={{ marginBottom: `${spacing.base}px` }}>
                {parents.map((p) => (
                  <div key={p.publicKey} style={{
                    ...
                    <Avatar avatar={p.avatarThumb} name={p.displayName || t('Parent')} size={32} />
                    <span style={{ width: '10px', height: '10px', borderRadius: '50%', flexShrink: 0, backgroundColor: p.isOnline ? colors.success : colors.border }} />
                    <span style={{ fontSize: '15px', fontWeight: '500', flex: 1, color: colors.text.primary }}>{p.displayName || t('Parent Device')}</span>
                    <span style={{ fontSize: '12px', color: colors.text.muted }}>{p.isOnline ? t('Connected') : t('Offline')}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {pairState === 'idle' && pairUiMode === 'initial' && (
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Button onClick={() => { window.callBare('haptic:tap'); setPairUiMode('methodPicker'); }}>
                  {parents.length > 0 ? t('Pair Another Parent') : t('Pair to Parent')}
                </Button>
              </div>
            )}
            ...
            {pairState === 'idle' && pairUiMode === 'methodPicker' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.sm}px` }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: `${spacing.sm}px`, marginTop: `${spacing.xs}px` }}>
                  ...
                  >
                    {sharingLink ? t('Generating...') : t('Share Link')}
                  </button>
            ...
            {pairState === 'idle' && pairUiMode === 'paste' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.sm}px` }}>
                ...
                />
                <div style={{ display: 'flex', gap: `${spacing.sm}px` }}>
                  <Button style={{ flex: 1 }} variant="secondary" onClick={() => { setPairUiMode('methodPicker'); setPasteUrl(''); }}>{t('Cancel')}</Button>
                  <Button style={{ flex: 1 }} onClick={() => { window.callBare('haptic:tap'); handlePasteAndPair(); }} disabled={!pasteUrl.trim()}>{t('Pair')}</Button>
                </div>
              </div>
            )}

            {pairState === 'connecting' && (
              <p style={{ color: colors.text.muted, fontSize: '14px', textAlign: 'center' }}>{t('Connecting to parent...')}</p>
            )}

            {pairState === 'success' && (
              <p style={{ color: colors.success, fontSize: '13px', margin: 0, textAlign: 'center' }}>{t('Pairing in progress...')}</p>
            )}
