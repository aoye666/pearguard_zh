...
const TYPE_META = {
  bypass:          { label: t('Bypass Attempt'),  icon: 'Warning' },
  ...
  app_installed:   { label: t('App Installed'),    icon: 'Plus' },
  app_uninstalled: { label: t('App Uninstalled'),  icon: 'Trash' },
  pin_override:    { label: t('PIN Override'),     icon: 'LockSimpleOpen' },
  pin_failure:     { label: t('PIN Guessing'),     icon: 'Warning' },
};
...
function metaFor(item) {
  if (item.type === 'time_request' && (item.requestType || 'approval') === 'approval') {
    return { label: item.origin === 'install' ? t('New App') : t('App Request'), icon: 'Plus' };
  }
...
function PendingRequestCard({ req, childPublicKey, onResolved }) {
  ...
  function approveLabel() {
    if (!isExtraTime) return t('Approve');
    if (!req.extraSeconds) return t('Grant Time');
    const mins = req.extraSeconds / 60;
    return mins >= 60 ? t('Grant {{hours}}h', { hours: mins / 60 }) : t('Grant {{minutes}}m', { minutes: mins });
  }
  ...
  const appLabel = req.appDisplayName || req.packageName || t('Unknown app');
  const title = isGeneralTime ? t('More screen time') : appLabel;
  ...
  const askedBecause = req.origin === 'install' ? t('just installed, not yet approved') : null;
        ...
        <div style={{ flex: 1 }}>
          ...
          {triggeredBy && (
            <div style={{ ...typography.caption, color: colors.text.secondary, marginBottom: '4px' }}>
              t('blocked while opening ') + triggeredBy
            </div>
          )}
          {askedBecause && (
            <div style={{ ...typography.caption, color: colors.text.secondary, marginBottom: '4px' }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginBottom: '2px' }}>
          ...
          <span style={{ ...typography.body, color: colors.text.primary }}>
            {item.appDisplayName || item.packageName || t('Unknown app')}
          </span>
          ...
          {item.type === 'pin_failure' && item.failCount
            ? <span style={{ ...typography.caption, color: colors.text.secondary }}>
                {t('{{count}} wrong attempts', { count: item.failCount })}
                {item.lockoutMs ? t(', locked {{minutes}} min', { minutes: Math.max(1, Math.round(item.lockoutMs / 60000)) }) + '' : ''}
              </span>
            : null}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: `${spacing.sm}px` }}>
          ...
          {item.type === 'time_request' && item.resolved
            ? <span style={{ ...typography.micro, color: colors.text.muted, fontStyle: 'italic' }}>
                {item.status === 'approved' ? t('Approved') : item.status === 'denied' ? t('Denied') : t('Resolved')}
              </span>
            : null}
        </div>
      </div>
      ...
      <div>
          ...
          <div style={{ width: '60px', flexShrink: 0, textAlign: 'right' }}>
            {clearableCount > 0 && !clearing && (
              <button
                ...
                aria-label={t("Clear activity log")}
                style={{
        ...
        {clearing && (
          <div style={{
            ...
            <span style={{ ...typography.caption, color: colors.text.secondary }}>
              {t('Clear {count} activity', { count: clearableCount }) + ' ' + t(clearableCount === 1 ? 'entry' : 'entries') + '?'}
              {t('Pending requests are kept.')}
            </span>
        ...
        {history.length === 0 ? (
          <div style={{ ...typography.body, color: colors.text.muted, textAlign: 'center' }}>{t('No activity yet.')}</div>
        ) : (
          history.map((item) => (
