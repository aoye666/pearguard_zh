...
const TABS = [
  { key: 'usage', label: t('Usage'), icon: 'ChartBar' },
  { key: 'apps', label: t('Apps'), icon: 'SquaresFour' },
  { key: 'activity', label: t('Activity'), icon: 'ListBullets' },
  { key: 'rules', label: t('Rules'), icon: 'Shield' },
  { key: 'advanced', label: t('Advanced'), icon: 'GearSix' },
];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: colors.surface.base }}>
      ...
      <div style={{
        ...
        <button
          ...
          aria-label={isPaused ? t('Free time active') : t('Pause protection')}
        >
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: `${spacing.xs}px` }}
          aria-label={locked ? t('Unlock device') : t('Lock device')}
        >
      ...
      <Modal
        ...
        title={t(`Lock ${child.displayName}'s device?`)}
        footer={<>
          <Button variant="secondary" onClick={() => { window.callBare('haptic:tap'); setConfirmLock(false); setLockMessage(''); }} style={{ flex: 1 }}>{t('Cancel')}</Button>
          <Button variant="danger" icon="LockSimple" onClick={handleConfirmLock} style={{ flex: 1 }}>{t('Lock')}</Button>
        </>}
      >
        <div style={{ textAlign: 'center', marginBottom: `${spacing.md}px` }}>
          {t(`This will immediately block all apps on ${child.displayName}'s device until you unlock it.`)}
        </div>
        <Input
          label={t('Message (optional)')}
          placeholder={t('Shown to your child on the block screen')}
          value={lockMessage}
