...
export default function InviteCard({ onConnected, onDismiss }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setScanState('error');
      setScanError(t('Timed out waiting for the child to connect. Make sure the child device is online, then try again.'));
    }, CONNECT_TIMEOUT_MS);
        ...
        {scanState === 'error' && (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: `${spacing.sm}px` }}>
              <Button onClick={() => { window.callBare('haptic:tap'); setScanState('idle'); }}>{t('Try Again')}</Button>
            </div>
          </>
        )}
        ...
        {scanState === 'idle' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.sm}px` }}>
            ...
            <div style={{ display: 'flex', gap: `${spacing.sm}px` }}>
              <Button style={{ flex: 1 }} variant="secondary" onClick={() => { setUiMode('methodPicker'); setPasteUrl(''); }}>{t('Cancel')}</Button>
              <Button style={{ flex: 1 }} onClick={handlePasteAndPair} disabled={!pasteUrl.trim()}>{t('Pair')}</Button>
            </div>
          </div>
        )}
    ...
    return (
      <div style={cardStyle}>
        ...
        {!invite ? (
          ...
          <>
            <p style={{ ...typography.caption, color: colors.text.secondary, margin: 0, marginBottom: `${spacing.base}px`, textAlign: 'center' }}>
              {t('Scan this QR code on the child\'s device.')}
            </p>
            ...
            <p style={{ ...typography.caption, color: colors.text.muted, fontStyle: 'italic', margin: 0, marginBottom: `${spacing.sm}px`, textAlign: 'center' }}>
              {t('Waiting for child to connect...')}
            </p>
          </>
        )}
    ...
    <div style={cardStyle}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: `${spacing.sm}px` }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: `${spacing.sm}px`, marginTop: `${spacing.xs}px` }}>
          ...
          >
            {sharing ? t('Generating...') : t('Share Link')}
          </button>
